...
interface GroupData { id: number; name: string; item_limit: number | null; export_mode: string; subgroups: { id: number; name: string; item_limit: number | null }[]; }
...
export default function AdminOrders() {
  ...
  const handleExportExcel = () => {
    ...
    for (const groupName of groupNamesList) {
      ...
      const exportMode = groupDef?.export_mode || "quantity";
      const itemLimit = groupDef?.item_limit || 0;
        ...
        for (const order of cycleOrders) {
          ...
          for (const item of groupItems) {
            for (let q = 0; q < item.quantity; q++) {
              selectedProducts.push(item.product_name_snapshot);
            }
          }

          let groupTotal = 0;
          groupItems.forEach(item => { groupTotal += parseFloat(item.unit_price) * item.quantity; });
          ...
          row.push(groupTotal);
          row.push(emp?.distribuicao || "");
          rows.push(row);
        }
      } else {
        headers = ["Matrícula", "Nome", "Setor", ...productList, `Total ${groupName}`];
      ...
      rows.sort((a, b) => (a[1] as string).localeCompare(b[1] as string, "pt-BR"));

      const sheetData = [headers, ...rows];
      const ws = XLSX.utils.aoa_to_sheet(sheetData);

      const totalColIdx = exportMode === "selection" ? headers.length - 2 : headers.length - 1;
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 bg-green-100 rounded-2xl flex items-center justify-center flex-shrink-0"><Download size={20} className="text-green-800" /></div>
              <h3 className="font-extrabold text-lg">Exportar planilha</h3>
            </div>
            <p className="text-gray-500 text-sm mb-5">
              Deseja baixar a planilha de pedidos de <strong>{selectedCycle ? `${MONTHS_FULL[selectedCycle.month - 1]} / ${selectedCycle.year}` : "—"}</strong> com {cycleOrders.length} pedido(s)?
            </p>
            <div className="flex gap-3">
              <button onClick={() => setShowExportConfirm(false)} className="flex-1 py-3 border border-gray-200 rounded-2xl font-semibold text-gray-600" data-testid="button-cancel-export">Cancelar</button>
              <button onClick={() => { setShowExportConfirm(false); handleExportExcel(); }} className="flex-1 py-3 bg-green-900 text-white rounded-2xl font-bold" data-testid="button-confirm-export">Baixar</button>
            </div>
          </div>
        </div>
      )}
