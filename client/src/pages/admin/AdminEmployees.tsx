...
import { Plus, Search, Edit2, Trash2, Unlock, Upload, X, Users, Key, UserCheck, UserX, RefreshCw, Filter, Camera } from "lucide-react";
...
export default function AdminEmployees() {
  ...
  const [selfieModal, setSelfieModal] = useState<{ id: number; name: string } | null>(null);
  ...
  const viewSelfie = async (emp: Employee) => {
    setSelfieModal({ id: emp.id, name: emp.name });
    setSelfieData(null);
    setSelfieLoading(true);
    try {
      const res = await fetch(`/api/employees/${emp.id}/selfie`, {
        headers: { "Authorization": "Bearer admin-token" },
      });
      if (res.ok) {
        const data = await res.json();
        setSelfieData(data.selfie_photo);
      } else {
        setSelfieData(null);
      }
    } catch {
    ...
    <div className="bg-gray-50">
            ...
            <div className="flex gap-1">
              {(emp as any).status === "inactive" ? (
                ...
                <>
                  ...
                  <button onClick={() => viewSelfie(emp)} className="w-8 h-8 bg-purple-50 rounded-xl flex items-center justify-center text-purple-600" title="Ver foto" data-testid={`button-selfie-${emp.id}`}><Camera size={14} /></button>
      ...
      {selfieModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-extrabold text-lg">Foto de Verificação</h3>
              <button onClick={() => setSelfieModal(null)} className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center" data-testid="button-close-selfie-modal"><X size={16} /></button>
            </div>
            <p className="text-sm font-medium text-gray-700 mb-3">{selfieModal.name}</p>
            {selfieLoading ? (
              <div className="flex items-center justify-center py-16">
                <div className="animate-spin w-8 h-8 border-4 border-green-200 border-t-green-700 rounded-full" />
              </div>
            ) : selfieData ? (
              <div className="rounded-2xl overflow-hidden border border-gray-200">
                <img src={selfieData} alt={`Selfie de ${selfieModal.name}`} className="w-full" data-testid="img-employee-selfie" />
              </div>
            ) : (
              <div className="text-center py-12 text-gray-400">
                <Camera size={40} className="mx-auto mb-2 opacity-40" />
                <p className="text-sm font-medium">Nenhuma foto registrada</p>
                <p className="text-xs mt-1">O funcionário ainda não realizou a verificação por foto.</p>
              </div>
            )}
          </div>
        </div>
