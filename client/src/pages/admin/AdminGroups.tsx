...
interface GroupData { id: number; name: string; description: string | null; item_limit: number | null; sort_order: number; export_mode: string; subgroups: SubgroupData[]; }
...
export default function AdminGroups() {
  ...
  const [form, setForm] = useState({ name: "", description: "", itemLimit: "", exportMode: "quantity" });
  ...
  const openEditGroup = (g: GroupData) => {
    setForm({ name: g.name, description: g.description || "", itemLimit: g.item_limit != null ? String(g.item_limit) : "", exportMode: g.export_mode || "quantity" });
    setEditId(g.id);
    setModal("edit");
  };

  const handleSaveGroup = () => {
    const payload = { name: form.name, description: form.description, item_limit: form.itemLimit ? parseInt(form.itemLimit) : null, sort_order: editId ? undefined : groups.length, export_mode: form.exportMode };
  ...
  return (
    <div className="bg-gray-50">
      <div className="px-4 pt-4 pb-2 flex items-center justify-between">
        ...
        <button onClick={() => { setForm({ name: "", description: "", itemLimit: "", exportMode: "quantity" }); setEditId(null); setModal("add"); }}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center">
          <div className="w-full max-w-2xl bg-white rounded-t-3xl p-5" style={{ paddingBottom: "calc(1.25rem + env(safe-area-inset-bottom, 0px))" }}>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Modalidade de exportação</label>
                <div className="flex gap-2 mt-1">
                  <button type="button" onClick={() => setForm({ ...form, exportMode: "selection" })}
                    className={`flex-1 py-3 rounded-2xl text-sm font-semibold border transition-all ${form.exportMode === "selection" ? "bg-green-900 text-white border-green-900" : "bg-gray-50 text-gray-600 border-gray-200"}`}
                    data-testid="button-export-mode-selection">
                    Seleção
                  </button>
                  <button type="button" onClick={() => setForm({ ...form, exportMode: "quantity" })}
                    className={`flex-1 py-3 rounded-2xl text-sm font-semibold border transition-all ${form.exportMode === "quantity" ? "bg-green-900 text-white border-green-900" : "bg-gray-50 text-gray-600 border-gray-200"}`}
                    data-testid="button-export-mode-quantity">
                    Quantidade
                  </button>
                </div>
                <p className="text-xs text-gray-400 mt-1">{form.exportMode === "selection" ? "Colunas por slot (ex: Item 1, Item 2...) com nome dos produtos escolhidos" : "Uma coluna por produto com a quantidade pedida"}</p>
              </div>
            </div>
