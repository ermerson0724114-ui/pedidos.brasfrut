...
export async function registerRoutes(
  ...
  app.post("/api/auth/search-by-name", async (req, res) => {
    const { name } = req.body;
    if (!name || typeof name !== "string" || name.trim().length < 2) {
      return res.status(400).json({ message: "Digite pelo menos 2 caracteres" });
    }
    const allEmps = await storage.getEmployees();
    const query = name.trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    const queryParts = query.split(/\s+/).filter(p => p.length > 0);

    const scored = allEmps
      .filter(e => e.status !== "inactive")
      .map(e => {
        const empName = (e.name || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
        let score = 0;
        if (empName === query) score = 100;
        else if (empName.startsWith(query)) score = 80;
        else if (empName.includes(query)) score = 60;
        else {
          const empParts = empName.split(/\s+/);
          let matched = 0;
          for (const qp of queryParts) {
            if (empParts.some(ep => ep.startsWith(qp) || ep.includes(qp))) matched++;
          }
          if (matched > 0) score = Math.round((matched / queryParts.length) * 50);
          if (score === 0) {
            for (const qp of queryParts) {
              for (const ep of empParts) {
                if (ep.length >= 3 && qp.length >= 3) {
                  const maxLen = Math.max(ep.length, qp.length);
                  let common = 0;
                  for (let i = 0; i < Math.min(ep.length, qp.length); i++) {
                    if (ep[i] === qp[i]) common++;
                    else break;
                  }
                  if (common >= 3) score = Math.max(score, Math.round((common / maxLen) * 30));
                }
              }
            }
          }
        }
        return { id: e.id, name: e.name, registration_number: e.registration_number, needsPassword: !e.password, score };
      })
      .filter(e => e.score > 0)
  ...
  app.post("/api/auth/create-password", async (req, res) => {
    const { employeeId, password, selfiePhoto } = req.body;
    if (!selfiePhoto) return res.status(400).json({ message: "Foto de verificação obrigatória" });
    const emp = await storage.updateEmployee(employeeId, { password, selfie_photo: selfiePhoto });
  ...
  app.get("/api/employees", async (_req, res) => {
    ...
    const cleaned = emps.map(({ selfie_photo, ...rest }) => rest);
    res.json(cleaned);
  });

  app.get("/api/employees/:id/selfie", async (req, res) => {
    ...
    res.json({ selfie_photo: emp.selfie_photo || null });
  });
  ...
  app.get("/api/cycle/current", async (_req, res) => {
    ...
    const cycleOverrideMonth = await storage.getSetting("cycleOverrideMonth");
    let isOpen = naturallyOpen;
    let overrideActive = false;
    let overrideTarget: "current" | "previous" | null = null;
    if (cycleOverride === "force_open" && !naturallyOpen) {
      isOpen = true;
      overrideActive = true;
      overrideTarget = (cycleOverrideMonth === "previous") ? "previous" : "current";
    ...
    if (overrideActive && overrideTarget === "previous") {
      if (month === 1) {
        cycleMonth = 12;
        cycleYear = year - 1;
      } else {
    ...
    const targetLastDay = getLastDayOfMonth(cycleYear, cycleMonth);
    ...
    if (!cycle) {
      cycle = await storage.createCycle({
        ...
        end_date: `${cycleYear}-${String(cycleMonth).padStart(2, "0")}-${targetLastDay}T23:59:59`,
    ...
    res.json({ cycle, isOpen, naturallyOpen, overrideActive, overrideTarget, daysRemaining, daysUntilOpen });
  });
